...
func (p *CubeParser) rotateCmd(hand Hand, degree int) *LinkedList {
	...
	if degree == 180 || degree == -180 {
		...
		} else if (hand == HandL && p.CubeStatus.LDegree < 0) || (hand == HandR && p.CubeStatus.RDegree < 0) {
			if degree < 0 {
				degree = -degree
			}
		}
		...
		// 90 度防缠绕
		if hand == HandL {
			if p.CubeStatus.LDegree >= 180 || p.CubeStatus.LDegree <= -180 {
				ConnectLists(cmdList, p.fixRotate(HandL, -p.CubeStatus.LDegree))
			}
		} else {
			if p.CubeStatus.RDegree >= 180 || p.CubeStatus.RDegree <= -180 {
				ConnectLists(cmdList, p.fixRotate(HandR, -p.CubeStatus.RDegree))
			}
		}
	}
